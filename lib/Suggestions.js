...
class Suggestions extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    const { isFocused, query, shouldRenderSuggestions, suggestions } = this.props;
  ...
  shouldRenderSuggestions = (query) => {
    const { isFocused, minQueryLength } = this.props;
    const minimumQueryLength = Number.isInteger(minQueryLength)
      ? minQueryLength
      : 2;
    return query.length >= minimumQueryLength && isFocused;
  };
  
  renderSuggestion = (item, i) => {
    const { classNames, handleClick, handleHover, query, selectedIndex, suggestionRenderer } = this.props;
    const suggestionLabel = (
      <span className={classNames.suggestionLabel} dangerouslySetInnerHTML={this.markIt(item, query)}/>
    );
    const content = suggestionRenderer ? suggestionRenderer(suggestionLabel) : suggestionLabel;
    ...
    return (
      <li
        ...
        className={
          i == selectedIndex ? classNames.activeSuggestion : ''
        }>
        <span className={classNames.suggestionContent}>{content}</span>
      </li>
    );
  };
  
  render() {
    const { classNames, query, suggestions, shouldRenderSuggestions } = this.props;
    ...
    return (
      <div
        ...
        <ul>
          {suggestions.map((item, i) => this.renderSuggestion(item, i))}
        </ul>
      </div>
    );
  }
