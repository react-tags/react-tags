...
class ReactTags extends Component {
  ...
  handleDelete(i, e) {
    const { handleDelete, resetInputOnDelete } = this.props;
  ...
  handleTagClick(i, e) {
    const { handleTagClick, resetInputOnDelete } = this.props;
  ...
  handlePaste(e) {
    const { allowAdditionFromPaste, delimiters, handleAddition } = this.props;
    ...
    string.split(regExp).forEach((tag) => handleAddition(tag));
  }
  ...
  getTagItems = () => {
    const { tags, labelField, removeComponent, readOnly, handleDrag, tagRenderer } = this.props;
