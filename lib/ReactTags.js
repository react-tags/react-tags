...
var ReactTags = React.createClass({
  propTypes: {
    ...
    resetStateAfterAddTag: React.PropTypes.bool,
  },
  ...
  addTag: function(tag) {
    ...
    // call method to add
    var hasToBeAdded = this.props.handleAddition(tag);

    // The hasToBeAdded variable now determines do we need to reset state,
    // or we need to wait new action from user.
    // For instance the tag is not included in the suggestions, thus app does not want to allow
    // user to add this tag to tags collection.

    // reset the state
    if (
      (this.props.resetStateAfterAddTag && hasToBeAdded) ||
      this.props.resetStateAfterAddTag === false
    ) {
      this.setState({
        query: "",
        selectionMode: false,
